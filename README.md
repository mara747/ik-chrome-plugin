...
| Interactive Brokers | kalibrováno (API) | pozice + Net Liq z interního API Client Portalu (`/v1/api/portfolio/…`, přihlášená session) vč. měny a burzy každé pozice → Yahoo tickery automaticky (RHM → RHM.DE, 9880 → 9880.HK); importuje i koupené opce (OCC symbol, kusy = kontrakty, prémie za akcii); záměrně bez DOM fallbacku — při selhání API dostane člen srozumitelnou chybu (nejčastěji vypršelá session → F5) |
| Trading 212 | kalibrováno (API) | z přihlášené session načte účet, otevřené pozice a metadata instrumentů; množství, `averagePrice` v původní měně instrumentu, celkovou hodnotu i hotovost. Hotovost zahrnuje hlavní balík (`investPot`), balík na utrácení (`spendingPot`), neinvestovanou Pie Cash a ověřenou rezervaci čekajících BUY (`blockedForStocks`); čekající SELL nemění hotovost ani pozice. Zrušený, částečně vyplněný nebo jinak neověřitelný pokyn a každý Pie pokyn (`valueOrder`) způsobí `cashValue: null` s upozorněním a web hotovost bezpečně dopočítá. Interní kód `TICKER_ZEMĚ_EQ` převádí na Yahoo symbol (např. nizozemský titul na `.AS`). Chyba instrumentu vypíše diagnostický kód a pouze bezpečný veřejný ticker/kód, měnu či burzu; nerozpoznanou hodnotu nahradí „neznámý/á“. Žádný HTML ani CSV fallback — neověřitelné pozice nebo instrumenty import bezpečně zastaví s bezpečnou diagnostikou a selhání API vyzve k obnovení stránky, zatímco nejednoznačná hotovost import nezastaví a odešle se jako `null` s upozorněním. |
...
| XTB | kalibrováno (pasivní Worker/WebSocket stream) | z již existujícího streamu XStation 5 načte právě vybraný účet, otevřené pozice, nativní vážené průměrné ceny, celkovou hodnotu a jednoznačně potvrzenou hotovost. U rozpoznaných čekajících BUY pokynů zahrne do hotovosti také brokerem vykázanou rezervaci `stockLock`; čekající SELL hotovost nemění a ignoruje se. Při nejednoznačné účetní identitě doplněk pošle `cashValue: null` a web klubu hotovost bezpečně dopočítá. Doplněk neotevírá vlastní spojení, neposílá XTB příkazy a nepoužívá DOM ani URL ikon. Neúplná, smíšená nebo neznámá data import bezpečně zastaví. Ambiguózní londýnské symboly nepřevádí obecně: pouze přesná, živě ověřená výjimka může doplnit chybějící nativní měnu a následný sdílený převod stále vyžaduje shodu brokeru, tickeru i měny. Kvůli odděleným kontextům Chromu používá dvojici `xtb-stream.js` (MAIN world od `document_start`, pozorování Workerů stránky) a `xtb.js` (ISOLATED world, registrace doplňku a jednorázový zabezpečený bridge). |
| George (Česká spořitelna) | kalibrováno (API) | z přihlášené session (Bearer token v `sessionStorage`, statický veřejný `web-api-key`) načte `/webapi/api/v3/netbanking/my/securities` — všechny majetkové účty a jejich tituly. Importují se **podílové fondy** (`securityType: FUND`): identita = ISIN, kusy, měna, **průměrná cena odvozená** `investedValue ÷ kusy` a snímek poslední ceny podílu (NAV), kterým web klubu fond oceňuje do dalšího importu. Importují se i **akcie** (`securityType: SHARE`, George Invest sdílí tentýž majetkový účet) jako běžné tickerové řádky s **ISINem místo symbolu** — George burzovní symbol vůbec neposílá (search i kotace jedou na ISIN + MIC), převod na Yahoo kotaci proto dělá až **tabulka aliasů na webu klubu**; neznámý ISIN zůstane poctivě bez ceny a založí žádost o analýzu. Název a kód burzy jdou do poznámky řádku. **Průměrka jen při shodě měn** (George vede investovanou částku v měně účtu) — cizoměnová akcie přijde bez průměrky s výzvou doplnit ji ručně. Ostatní typy (ETF, dluhopisy, certifikáty) se hlasitě přeskočí. Úspěšný, ale neúplný import navíc nabídne **kalibrační diagnostiku** (`partial_import`): identita přeskočených titulů (ISIN, název, typ, burza, měna) a jména + měny peněžních polí cizoměnových akcií — nikdy kusy ani částky. Hotovost se posílá explicitně (`balance − Σ titulů`, typicky 0) — majetkový účet hotovost nedrží, pokyny se vypořádávají z běžného účtu a čekající pokyny se ignorují |

Vzácné symboly, které nelze spolehlivě převést obecným pravidlem, opravuje až
společná přesná tabulka v `lib/normalize.js`. Klíčem je broker + normalizovaný
...
Stejné tlačítko se nenápadně nabídne i po **úspěšném, ale neúplném** importu
(kód `partial_import`) — tam smí navíc nést identitu titulů, kterých se problém
týká (ISIN, název, typ, burza, měna a jména + měny peněžních polí); člen ji vidí
v náhledu a potvrzuje druhým tlačítkem. Velikosti — kusy, částky, hodnoty,
výkonnost — se neposílají nikdy. Primární tlačítko „Odeslat do webu klubu"
diagnostika neblokuje, poslat jde obojí.
