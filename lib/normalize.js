    ...
    chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
      ...
      if (msg?.type === "IK_SCRAPE") {
        Promise.resolve()
          ...
          .then((res) => {
            if (!res?.ok) {
              sendResponse(res?.diagnostic || !globalThis.IKDiagnostics ? res : {
            ...
            try {
              sendResponse({ ...res, payload: applyBrokerTickerOverrides(res.payload) });
            } catch (error) {
              sendResponse({
                ok: false,
                error: "Načtená data se nepodařilo bezpečně převést.",
                diagnostic: globalThis.IKDiagnostics?.failure({
                  phase: "normalize", errorCode: "unexpected_error",
                  error, step: `${def.broker}.normalize`,
                }),
              });
            }
          })
